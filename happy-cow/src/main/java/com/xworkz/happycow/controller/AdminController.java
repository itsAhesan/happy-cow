...
public class AdminController {
  ...
  public String adminLoginProcess(
    ...
    try {
      ...
      log.debug(
          "Preloaded {} notifications into session for {}",
          (notifs != null ? notifs.size() : 0),
          adminDTO.getEmailId());
    } catch (Exception e) {
    ...
    log.info(
        "Login successful for email: {} in {}ms", email, System.currentTimeMillis() - startTime);
  ...
  public String updateProfile(@ModelAttribute AdminDTO adminDTO, HttpSession session, Model model) {
  ...
  public String logout(HttpSession session, RedirectAttributes redirectAttributes) {
    ...
    if (loggedInAdmin != null) {
      log.info(
          "Admin '{}' (ID: {}) logged out", loggedInAdmin.getEmailId(), loggedInAdmin.getAdminId());
    } else {
